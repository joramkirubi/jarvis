...
JARVIS_WELCOME_PHRASE = (
    "Welcome back, Joram. All systems standing by. "
    "Shall we work on Pesa IQ, tackle some code, "
    "or start another project before finishing the last one? "
    "I've prepared your workspace, sir. The ambition, as always, is your department."
)
...
def _chrome_new_window_wait_timeout_s() -> float:
    try:
        return max(
            3.0, float((os.environ.get("CHROME_NEW_WINDOW_WAIT_S") or "25").strip())
        )
    except ValueError:
        return 25.0
...
def open_binance_btc_in_chrome() -> None:
    if not OPEN_BINANCE_BTC_IN_CHROME:
        return
    url = (
        os.environ.get("BINANCE_BTC_URL") or "https://www.binance.com/en/trade/BTC_USDT"
    ).strip()
    pos: tuple[int, int] | None = None
    size: tuple[int, int] | None = None
    fs = OPEN_CHROME_FULLSCREEN
...
def open_cursor_window() -> None:
    ...
    try:
        if FOCUS_EXISTING_CURSOR_ON_DOUBLE_CLAP:
            focused = sys.platform == "win32" and _focus_existing_cursor_window_win32()
            if not focused:
...
def main() -> int:
    ...
    try:
        ...
        log.error(
            "If PortAudio fails, install/repair drivers or try another SAMPLE_RATE."
        )
        return 1
